/**
 * credit to https://codepen.io/justgooddesign/pen/AzaLmZ
 */
// http://suncalc.net/

import { colord } from "colord";

type ColorGrad = { color: string; position: number }[];

// gradient colors from https://cdpn.io/rDEAl
const grads: ColorGrad[] = [
  [
...
function toCSSGradient(data: ColorGrad) {
  ...
  for (var i = 0; i < len; i++) {
    var item = data[i];
    css += " #" + item.color + " " + item.position * positionOffset + "%";
    if (i < len - 1) css += ",";
  }
  return css + ")";
}
...
function colorStrToColor(cl: string) {
  return colord(`#${cl}`);
}

export function getColorGradientByIndex(nIdx: number) {
  const grad = grads[nIdx];
  return colorStrToColor(grad[grad.length - 1].color);
}

export function updateCSSGradientBasedOnNow() {
